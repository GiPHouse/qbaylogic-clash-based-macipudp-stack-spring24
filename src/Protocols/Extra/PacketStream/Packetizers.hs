...
Description : Generic packetizers for appending and stripping headers to and from the beginning of packets.
-}
module Protocols.Extra.PacketStream.Packetizers
  ( packetizerC
  , depacketizerC
  ...
  where
    ...
    nextSt
      = case (_deCounter == 0, _last) of
          ...
          (True, Just idx)
            -> DeLastForward nextAborted nextParseBuf fwdBuf nextCounter (idx - natToNum @(headerBytes `Mod` dataWidth))
          (True, Nothing)
    ...
    outReady | DeLastForward {} <- nextSt = False
             | otherwise = True
...
depacketizerT toMetaOut st@DeLastForward{..} (fwdIn, bwdIn) = (nextStOut, (bwdIn, Just outPkt))
  where
    ...
    outPkt = PacketStreamM2S
               ...
               , _data = _deFwdBuf ++ repeat @(dataWidth - DeForwardBufSize headerBytes dataWidth) defaultByte
    ...
    outCircuit
      = case (modProof, divProof) of
          (SNatLE, SNatLE) -> case compareSNat (SNat @(DeForwardBufSize headerBytes dataWidth)) (SNat @dataWidth) of
            SNatLE -> mealyB (depacketizerT @headerBytes toMetaOut) (DeParse False (repeat undefined) (repeat undefined) maxBound)
...
packetizeFromDfT toMetaOut toHeader DfIdle (Data dataIn, bwdIn) = (nextStOut, (bwdOut, Just outPkt))
  where
    rotatedHdr = rotateRightS (bitCoerce (toHeader dataIn)) (SNat @(DfHeaderBufSize headerBytes dataWidth))
...
dfDeInitialState :: forall (a :: Type) (headerBytes :: Nat) (dataWidth :: Nat)
   . KnownNat dataWidth
  => KnownNat headerBytes
  => 1 <= dataWidth
