...
-}
module Clash.Cores.Ethernet.Depacketizer (depacketizerC) where
...
equivalentBufSizes
  :: forall (headerBytes :: Nat) (dataWidth :: Nat)
   . (1 <= dataWidth)
    :- ( ParseBufSize headerBytes dataWidth
          + dataWidth
          ~ headerBytes
          + ForwardBufSize headerBytes dataWidth
       )
...
deriving instance
  (Show metaOut, DepacketizerCt headerBytes dataWidth)
  => Show (DepacketizerState metaOut headerBytes dataWidth)

deriving instance
  (ShowX metaOut, DepacketizerCt headerBytes dataWidth)
  => ShowX (DepacketizerState metaOut headerBytes dataWidth)

deriving instance
  (NFDataX metaOut, DepacketizerCt headerBytes dataWidth)
...
depacketizerT toMetaOut st@Parse{..} (Just pkt@PacketStreamM2S{..}, bwdIn) = (nextStOut, (PacketStreamS2M readyOut, fwdOut))
 where
  ...
  (headerBytes, forwardBytes) =
    case equivalentBufSizes @headerBytes @dataWidth of
      Sub Dict -> splitAt (SNat @headerBytes) combined

  metaOut = toMetaOut (bitCoerce headerBytes) _meta
  dataOut =
    forwardBytes
      ++ ( repeat defaultByte :: Vec (dataWidth - ForwardBufSize headerBytes dataWidth) (BitVector 8)
         )
  ...
  outCircuit =
    case (divProof, modProof) of
      (SNatLE, SNatLE) -> case compareSNat (SNat @(ForwardBufSize headerBytes dataWidth)) (SNat @dataWidth) of
        SNatLE ->
          mealyB (depacketizerT @headerBytes toMetaOut) (Parse False (repeat undefined) maxBound)
        _ ->
          errorX
