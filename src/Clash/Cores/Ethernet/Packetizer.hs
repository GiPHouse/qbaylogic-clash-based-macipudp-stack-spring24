...
-}
module Clash.Cores.Ethernet.Packetizer (packetizerC) where
...
packetizerC toMetaOut toHeader = fromSignals outCircuit
 where
  outCircuit = case compareSNat (SNat @(ForwardBufSize headerBytes dataWidth)) (SNat @dataWidth) of
    ...
    _ ->
      errorX
        "packetizer0: Absurd, Report this to the Clash compiler team: https://github.com/clash-lang/clash-compiler/issues"
