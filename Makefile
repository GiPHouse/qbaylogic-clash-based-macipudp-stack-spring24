...
verilog=verilog/Clash.Lattice.ECP5.Colorlight.UdpCore.udpCore/udpCore.v
netlist=netlist/synth.json
pnr=netlist/pnr.cfg
bitstream=netlist/clash-eth.bit

${verilog}: ${HASKELL_SOURCES}
	cabal run clash -- Clash.Lattice.ECP5.Colorlight.UdpCore --verilog -g -fclash-clear
...
${netlist}: ${verilog}
	...
	yosys \
		...
		-p "synth_ecp5 -no-rw-check -abc2 -top udpEcho" \
		-p "ecp5_infer_bram_outreg" \
		-p "write_json ${netlist}" \
		verilog/Clash.Lattice.ECP5.Colorlight.UdpCore.udpCore/*.v \
		udpEcho.v
